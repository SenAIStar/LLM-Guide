...
def rope_test():
    ...
    # 直接调用整合后的compute_rope（无需提前预计算参数）
    queries_rot = compute_rope(queries)
    keys_rot = compute_rope(keys)

    # 计算点积并可视化
    q_k_dot = queries_rot[0,0,[0]] @ keys_rot[0,0,:,:].transpose(1,0)
    distances = np.arange(0, context_len)
    ...
    plt.title('Dot Product of q and k after applying RoPE')
    plt.legend()
    plt.grid(True)
    plt.savefig('q_k_dot_product_plot.png')
    print("Plot saved as q_k_dot_product_plot.png")
